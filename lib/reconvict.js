...
let reconvict = function reconvict(def, opts) {
  ...
  let rv = {
    ...
    /**
     * Adds a reloader to this config.
     */
    reloader: function(reloaderFactory) {
    ...
    loadFile: function(paths) {
      let self = this;
      const overlayChanged = new Set();
      if (!Array.isArray(paths)) paths = [paths];
      paths.forEach(function(path) {
        // Support empty config files #253
        const result = loadFile(path);
        if (result) {
          const changed = overlay('', result, self._instance, self._schema);

          changed.forEach(key => overlayChanged.add(key))
        }
      });
      // environment and arguments always overrides config files
      // TODO: What happens in a set -> load sequence?
