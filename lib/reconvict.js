...
function set(instance, k, v) {
  v = coerce(k, v, instance._schema, instance);
  let path = k.split('.');
  let childKey = path.pop();
  let parentKey = path.join('.');
  let parent = walk(instance._instance, parentKey, true);
  const changed = !util.isDeepStrictEqual(parent[childKey], v);
  parent[childKey] = v;
  ...
  Object.keys(o._env).forEach(function(envStr) {
    if (env[envStr] !== undefined) {
      ...
      ks.forEach(function(k) {
        const changed = set(o, k, env[envStr]);
...
function importArguments(o) {
  ...
  Object.keys(o._argv).forEach(function(argStr) {
    ...
    if (argv[argStr] !== undefined) {
      const changed = set(o, k, String(argv[argStr]));
...
function overlay(parentPath, from, to, schema) {
  const changedKeys = new Set();

  Object.keys(from).forEach(function(k) {
    // leaf
    const fullPath = parentPath == '' ? k : `${parentPath}.${k}`;

    if (Array.isArray(from[k]) || !isObj(from[k]) || !schema || schema.format === 'object') {
      const changed = !util.isDeepStrictEqual(from[k], to[k])
      ...
      const childChanged = overlay(fullPath, from[k], to[k], schema.properties[k]);

      childChanged.forEach(key => changedKeys.add(key));
    }
  });
...
function hasChildren(flatInstance, path) {
  return Object.keys(flatInstance)
    .some(key => (key !== path) && (key.startsWith(path)))
}

function proxyValue(flatInstance, path, value) {
  const proxy = new Proxy({}, {
    get: function(target, prop) {
      const concatPath = `${path}.${prop}`

      if (hasChildren(flatInstance, concatPath)) {
        return proxyValue(flatInstance, concatPath, value[prop])
      } else {
        return value[prop]
      }
    },
...
function proxiedWalk(instance, path) {
  const flatInstance = flatten(instance);

  const value = walk(instance, path);

  if (hasChildren(flatInstance, path)) {
    return proxyValue(flatInstance, path, value);
  } else {
    return value;
  }
}

function emitChanged(instance, paths) {
  if (paths.size > 0) {
    instance.emit('change', paths, instance);
  }
}
...
function loadFile(instance, paths) {
  const overallChanged = new Set();
  ...
  paths.forEach(function(path) {
    ...
    if (result) {
      const changed = overlay('', result, instance._instance, instance._schema);

      changed.forEach(key => overallChanged.add(key));
    }
  });
 ...
 */
let reconvict = function reconvict(def, opts) {
  ...
  let rv = {
     ...
     * reconvict function or process.argv if no override was passed.
     */
    getArgs: function() {
     ...
     * reconvict function or process.env if no override was passed.
     */
    getEnv: function() {
     ...
     */
    get: function(path) {
      return proxiedWalk(this._instance, path);
    },
     ...
     */
    set: function(k, v) {
      const changed = set(this, k, v);

      if (changed) {
        emitChanged(this, new Set([k]))
      }
     ...
     */
    load: function(conf) {
      const overlayChanged = overlay('', conf, this._instance, this._schema);
      // environment and arguments always overrides config files
      // TODO: What happens in a set -> load sequence?
      const envRollback = importEnvironment(rv);
      const argRollback = importArguments(rv);

      const actualChanged = difference(difference(overlayChanged, envRollback), argRollback);

      emitChanged(this, actualChanged);
    ...
    /**
     * Loads and merges a JavaScript object into config without
     * respect to environment and argument values.
     */
    forceLoad: function(conf) {
      const changedKeys = overlay('', conf, this._instance, this._schema);

      emitChanged(this, changedKeys);

      return this;
    },

    /**
     * Loads and merges one or multiple configuration files into config without
     * respect to environment and argument values.
     */
    forceLoadFile: function(paths) {
      const changedKeys = loadFile(this, paths);
    ...
    /**
     * Loads and merges one or multiple configuration files into config.
     */
    loadFile: function(paths) {
      ...
      const actualChanged = difference(difference(changedKeys, envRollback), argRollback);
    ...
    reloader(reloaderFactory) {
      this._reloaders.push(reloaderFactory(this));
  ...
  Object.setPrototypeOf(rv, new EventEmitter());
 ...
 */
reconvict.addFormat = function(name, validate, coerce) {
 ...
 */
reconvict.addFormats = function(formats) {
  Object.keys(formats).forEach(function(type) {
    reconvict.addFormat(type, formats[type].validate, formats[type].coerce);
  });
};
 ...
 */
reconvict.addParser = function(parsers) {
